...
<SectionContainer
  title='Assignments'
  description='Track tasks and upcoming deadlines.'
...
>
  {#if sortedAssignments.length === 0}
    <div class='mt-1 flex flex-row items-center gap-3 b-3 rounded-md b-dashed p-3 text-muted'>
      ...
      <div>No assignments yet. Add one.</div>
    </div>
  {:else}
    ...
    <Dialog.Content>
      ...
      <div class='w-full flex flex-col gap-4'>
        <div class='flex flex-col gap-2'>
          ...
          <Input id='title' type='text' bind:value={title} placeholder='Read Chapter 5' />
        </div>
        <div class='grid grid-cols-2 gap-3'>
          <div class='flex flex-col gap-2'>
            <Label for='subject'>Subject</Label>
            <Input id='subject' type='text' bind:value={subject} placeholder='History' />
          </div>
          <div class='flex flex-col gap-2'>
        ...
        <Dialog.Close disabled={!isAddReady} class='w-full'>
          <Button class='w-full' disabled={!isAddReady} onclick={addAssignment}>Add</Button>
        </Dialog.Close>
      </div>
    </Dialog.Content>
  </Dialog.Root>
  ...
  <Dialog.Root bind:open={isEditOpen}>
    <Dialog.Content>
      ...
      <div class='w-full flex flex-col gap-4'>
        ...
        <div class='flex flex-col gap-2'>
          <Label for='edescription'>Notes</Label>
          <Textarea
            id='edescription'
        ...
        <div class='flex flex-row items-center justify-between border border-input rounded-md px-3 py-2'>
          <div class='flex flex-col'>
            ...
            <span class='text-xs text-muted'>Toggle when the assignment is done.</span>
          </div>
          <button
            type='button'
            ...
            aria-label='Assignment completion status'
        ...
        <div class='flex flex-col gap-2'>
          <Dialog.Close disabled={!isEditReady}>
            <Button class='w-full' disabled={!isEditReady} onclick={saveAssignment}>Save</Button>
          </Dialog.Close>
